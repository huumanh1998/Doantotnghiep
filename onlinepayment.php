...
if(isset($_GET['order'])){
        ...
        if($get_product_cart){
            ...
            while($result = $get_product_cart->fetch_assoc()){
                ...
                array_push($productNames2,$result['price']);
                array_push($productNames3, ($result['quantity']*$result['price']) + ($result['quantity']*$result['price']) * 0.05);
            }
        }
        ...
        $customer_id = Session::get('customer_id');
        $insertOrder = $ct->insertOrderOn($customer_id);
        $delCart = $ct->del_all_data_cart();
        header('Location:success.php');
    }
